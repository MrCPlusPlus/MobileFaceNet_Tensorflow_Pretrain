...
def inverted_block(net, input_filters, output_filters, expand_ratio, stride, scope=None):
    ...
    res_block = slim.conv2d(inputs=net, num_outputs=input_filters * expand_ratio, kernel_size=[1, 1], scope=scope+'_conv_sep')
    # depthwise conv2d
    if stride == 2:
      paddings = tf.constant([[0,0],[1,1],[1,1],[0,0]])
      res_block = tf.pad(res_block,paddings,"CONSTANT")
      res_block = slim.separable_conv2d(inputs=res_block, num_outputs=None, kernel_size=[3, 3], stride=stride, padding='VALID', depth_multiplier=1.0, normalizer_fn=slim.batch_norm, scope=scope+'_conv_dw')
    else:
      res_block = slim.separable_conv2d(inputs=res_block, num_outputs=None, kernel_size=[3, 3], stride=stride, depth_multiplier=1.0, normalizer_fn=slim.batch_norm, scope=scope+'_conv_dw') 
    res_block = slim.conv2d(inputs=res_block, num_outputs=output_filters, kernel_size=[1, 1], activation_fn=None, scope=scope+'_conv_proj')
    # stride 2 blocks
    if stride == 2:
        return res_block
    # stride 1 block
        ...
        if input_filters != output_filters:
            net = slim.conv2d(inputs=net, num_outputs=output_filters, kernel_size=[1, 1], activation_fn=None, scope=scope+'_error_dim')
...
def mobilenet_v2_base(inputs,
  ...
  #np.random.seed(5)
  #weights = np.random.rand(3,3,3,64)
  #tf_weights = tf.Variable(weights, dtype=np.float32, name='conv_1/weights')
  ...
  with tf.variable_scope(scope, 'MobileFaceNet', [inputs]):
    with slim.arg_scope([slim.conv2d, slim.separable_conv2d], padding='SAME'):
      ...
      net = slim.conv2d(net, 64, [3, 3], stride=2, normalizer_fn=slim.batch_norm, padding='VALID', scope=end_point)
      ...
      net = slim.separable_conv2d(inputs=net, num_outputs=None, kernel_size=[3, 3], stride=1,
                                  depth_multiplier=1.0, normalizer_fn=slim.batch_norm, scope=end_point)
      end_points[end_point] = net

      end_point = 'dconv_23'
      input_filters = net.shape[3].value
      net = inverted_block(net, input_filters, 64, 2, 2, end_point)
      end_points[end_point] = net

      end_point = 'res_3'
      input_filters = net.shape[3].value
      for index in range(4):
        suffix = '_block' + str(index)
        net = inverted_block(net, input_filters, 64, 2, 1, end_point+suffix)
      end_points[end_point] = net

      end_point = 'dconv_34'
      input_filters = net.shape[3].value
      net = inverted_block(net, input_filters, 128, 4, 2, end_point)
      end_points[end_point] = net
      ...
      for index in range(6):
        suffix = '_block' + str(index)
        net = inverted_block(net, input_filters, 128, 2, 1, end_point+suffix)
      end_points[end_point] = net
      ...
      end_point = 'conv_6sep'
      net = slim.conv2d(net, 512, [1, 1], stride=1, normalizer_fn=slim.batch_norm, scope=end_point)
      end_points[end_point] = net
...
def mobilenet_v2(inputs,
  ...
  with tf.variable_scope(scope, 'MobileFaceNet', [inputs], reuse=reuse) as scope:
    with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
      net, end_points = mobilenet_v2_base(inputs, scope=scope, min_depth=min_depth)
      ...
      with tf.variable_scope('Logits'):
        ...
        logits = slim.fully_connected(net, bottleneck_layer_size, activation_fn=None, scope='pre_fc1')
        logits = slim.batch_norm(logits, scope='fc1')
...
def mobilenet_v2_arg_scope(is_training=True,
  ...
  weights_init = tf.contrib.layers.xavier_initializer(uniform=True)
...
def get_logits(features, num_outputs):
    logits = tf.contrib.slim.fully_connected(features, num_outputs, activation_fn=None, scope='fc7')
    return logits
